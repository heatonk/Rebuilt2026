...
public class TurretDynamicCommand extends Command {
  ...
  public void execute() {
    ...
    Logger.recordOutput(PREFIX + "Velocity (rots)", velocitySignal.getValueAsDouble());
    Logger.recordOutput(PREFIX + "Acceleration (rots^2)", accelerationSignal.getValueAsDouble());
  ...
  public void end(boolean interrupted) {
    ...
    Logger.recordOutput(PREFIX + "kA (Amps per rots^2)", kA);
    Logger.recordOutput(PREFIX + "Input kS (Amps)", inputKS);
    Logger.recordOutput(PREFIX + "Input kV (Amps per rots)", inputKV);
